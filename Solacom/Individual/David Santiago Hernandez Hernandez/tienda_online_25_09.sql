...
select max(precioProducto)  from Producto;


select * from Ventas inner join Cliente on Ventas.idClienteFK = Cliente.idCliente;

select v.*, c.nombreCliente  from Ventas v inner join Cliente c on v.idClienteFK = c.idCliente;



select c.nombreCliente, v.idClienteFK, Ventas_Producto.total from Ventas v inner join Cliente c on Ventas.idClienteFK = Cliente.idCliente inner join Ventas_Producto on Ventas.idOrden=Ventas_Producto.idVentaFK where total=max(total);
...
-- consultar usuario y cliente de venta especifica
-- consultar los productos que compro un cliente especifico 
-- consultar todos los clientes que han hecho compras (tabla ventas o orden)
